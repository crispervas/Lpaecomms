...
`tailwind.config.js`'s `content` globs scan **both** `src/views/**/*.ejs` and
`src/public/js/**/*.js`. The second glob exists because some browser scripts
set complete Tailwind class names at runtime (e.g. `mashup-map.js` switching
the map container to its amber "unavailable" state) — those class names live
